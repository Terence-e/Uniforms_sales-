-- record_return() gains the policy verdict (A-FR-8.9 to A-FR-8.11).
--
-- Replaces the signature from 20260101002900, which had no notion of policy.
-- Dropped rather than replaced: `create or replace` with a different argument
-- list creates an OVERLOAD, and PostgREST would then have two candidates and
-- refuse the call as ambiguous.
--
-- What changes:
--   * the verdict is computed here and stored on the row
--   * an out-of-policy return requires an override reason, separate from the
--     ordinary reason every return already carries
--   * an override writes its own audit row, flagged, so A-FR-8.12 can count them
--
-- What does NOT change: nothing blocks. Out of policy is a warning with a
-- price -- an explanation, on the record, attributable to a person.
...
begin
  ...
  loop
    ...
    -- Back into sellable stock (A-FR-8.2).
