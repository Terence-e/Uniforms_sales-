...
--
-- A seller sees and creates only their own sales; admins see everything.
-- Nothing is deletable and only admins may amend a recorded sale -- a sales
-- ledger that staff can quietly rewrite is not a ledger.
  ...
  using (
    exists (
      ...
      where s.id = sale_items.sale_id
        and (s.seller_id = (select auth.uid()) or public.is_admin())
    )
  );
